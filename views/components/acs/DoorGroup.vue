...
@Component
/// 1) class name
export default class DoorGroup extends BasicFormQuick implements IFormQuick2 {
    ...
    getReadersCount(doors:any){
        ...
        for(let item of doors){
            count += item.readerin?item.readerin.filter(x=>x.status==1).length:0;
            count += item.readerout?item.readerout.filter(x=>x.status==1).length:0;
        }
        return count;
    }
}
